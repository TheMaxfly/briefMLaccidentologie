...
def render():
    ...
    age_selected = st.selectbox("Classe d'age", options=age_options, index=age_index, key="age_input")
    if age_selected:
    ...
    catv_selected = st.selectbox("Famille de vehicule", options=catv_options, index=catv_index, key="catv_input")
    if catv_selected:

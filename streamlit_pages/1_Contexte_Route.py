...
# Page configuration
st.set_page_config(
    page_title="Page 1 - Contexte Route",
    page_icon="🛣️",
    layout="centered"
)
...
# Header
st.title("🛣️ Page 1 : Contexte Route")
st.caption("Informations sur le département, la route et l'agglomération")

# Get reference data
ref_data = session_state.get_reference_data()

# Field 1: Département (dep)
st.subheader("Département")
...
# Field 2: Agglomération (agg)
st.subheader("Agglomération")
...
agg_selected = st.selectbox(
    ...
    index=agg_index,
    key="agg_input",
    help="Accident en ou hors agglomération"
)
if agg_selected:
...
# Field 3: Catégorie de route (catr)
st.subheader("Catégorie de route")
...
# Field 4: VMA bucket
st.subheader("Vitesse maximale autorisée")

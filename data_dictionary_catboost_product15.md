# Référentiel d’entrée — Modèle final CatBoost (product15)
...
| `minute` | numérique (0–59) | Minute (0–59) de l’heure de l’accident (extrait de `hrmn`). |
...
- Valeurs attendues : entier **0 à 59**.
- Exemple : si l’heure est `05:40`, alors `minute = 40`.
...
- Pour `minute`, soit demander directement la minute, soit demander `HH:MM` et parser en minute côté API.
